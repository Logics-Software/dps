...
if (!function_exists('getSortIconLaporanHarga')) {
    function getSortIconLaporanHarga($column, $currentSortBy, $currentSortOrder) {
        $config = require __DIR__ . '/../../config/app.php';
        $baseUrl = rtrim($config['base_url'], '/');
        if (empty($baseUrl) || $baseUrl === 'http://' || $baseUrl === 'https://') {
            $baseUrl = '/';
    ...
    <div class="col-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Laporan Daftar Harga</li>
            </ol>
        </nav>
    </div>
</div>
        ...
        <div class="card">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Laporan Daftar Harga</h4>
            ...
            <div class="card-body">
                <form method="GET" action="/laporan/daftar-harga" class="mb-3">
                    <div class="row g-2 align-items-end search-filter-card">
                        ...
                        <div class="col-6 col-md-1">
                            <a href="/laporan/daftar-harga" class="btn btn-outline-secondary">Reset</a>
                        </div>
                    </div>
                </form>
                    ...
                    <table class="table table-striped table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                <th>No</th>
                                <th>
                                    <a href="<?= getSortUrlLaporanHarga('namabarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Nama Barang
                                        <?= getSortIconLaporanHarga('namabarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>
                                    Satuan
                                ...
                                <th>
                                    <a href="<?= getSortUrlLaporanHarga('pabrik', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Pabrik
                                        <?= getSortIconLaporanHarga('pabrik', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>
                                    Harga Jual
