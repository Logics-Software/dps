...
class ApiWebAuthnController extends Controller {
    ...
    public function authenticationStart() {
        try {
            ...
            error_log("WebAuthn authentication start - User ID: {$user['id']}, Username: {$username}, Credentials count: " . count($credentials));
            
            if (empty($credentials)) {
                // Return success with empty credentials - allows frontend to check properly
                error_log("WebAuthn: No credentials found for user {$username} (ID: {$user['id']})");
                echo json_encode(['success' => true, 'options' => ['allowCredentials' => []]]);
                return;
            }
            ...
            // Log for debugging
            error_log("WebAuthn: Returning options with " . count($options['allowCredentials']) . " credentials");
