...
class WebAuthnHelper {
    ...
    static async hasCredentials(username) {
        try {
            console.log('WebAuthnHelper.hasCredentials called with username:', username);
            ...
            if (!response.ok) {
                // If 404 or 400, user has no credentials
            ...
            // Check if request was successful and has credentials
            if (data.success && data.options) {
                const allowCredentials = data.options.allowCredentials;
                console.log('allowCredentials:', allowCredentials);
                
                // If allowCredentials exists and has items, user has credentials
                if (allowCredentials && Array.isArray(allowCredentials)) {
                    const hasCreds = allowCredentials.length > 0;
                    console.log('Has credentials:', hasCreds, 'Count:', allowCredentials.length);
                    return hasCreds;
                }
                // If allowCredentials is empty array, user has no credentials
                console.log('allowCredentials is empty or not an array');
                return false;
            }
            
            console.log('Response not successful or missing options');
            return false;
        } catch (error) {
