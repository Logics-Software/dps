...
Public Function ParseJSONValue(jsonString As String, key As String, Optional shouldDecode As Boolean = False) As String
    ...
    If startPos > 0 Then
        ...
        Else
            ...
            If Mid(jsonString, startPos, 4) = "null" Then
                endPos = startPos + 4
                value = ""
            Else
                endPos = startPos
...
Public Function IsAPISuccess(response As String) As Boolean
    ...
    ' Check if response is in ERROR format
    If Left(response, 5) = "ERROR" Then
        IsAPISuccess = False
        Exit Function
    End If
    
    ' Parse success field from JSON
...
Public Function GetAPIErrorMessage(response As String) As String
    ...
    ' Check if response is in ERROR format: "ERROR:status|json_response"
    If Left(response, 5) = "ERROR" Then
        ' Extract status code and JSON part
        pipePos = InStr(response, "|")
        If pipePos > 0 Then
            statusCode = Mid(response, 7, pipePos - 7) ' Skip "ERROR:" (6 chars)
            jsonPart = Mid(response, pipePos + 1)
            
            ' Try to parse message from JSON
            message = ParseJSONValue(jsonPart, "message")
            
            ' If no message found, try to get more details
            If message = "" Then
                ' Try to get error details or use status code
                Dim errorDetail As String
                errorDetail = ParseJSONValue(jsonPart, "error")
                If errorDetail <> "" Then
                    message = "HTTP " & statusCode & ": " & errorDetail
                Else
                    message = "HTTP " & statusCode & ": " & Left(jsonPart, 100)
                End If
            Else
                message = "HTTP " & statusCode & ": " & message
            End If
        Else
            ' No pipe found, return the whole error
            message = response
        End If
    Else
        ' Normal JSON response, try to parse message
        message = ParseJSONValue(response, "message")
        If message = "" Then
            ' Try alternative error fields
            message = ParseJSONValue(response, "error")
            If message = "" Then
